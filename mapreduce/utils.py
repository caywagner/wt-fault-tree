...
def udp_recv(sock, buf_size=4096):
    """Recieve message from adr:port via UDP connection."""
    try:
        message_str = sock.recv(buf_size)
    except socket.timeout:
        return None
    try:
        message_dict = json.loads(message_str)
...
def udp_send(message, port, adr="localhost"):
    """Send message to adr:port via UDP connection."""
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    sock.connect((adr, port))
    sock.sendall(json.dumps(message).encode('UTF-8'))

    logging.info("UDP send connection to %s:%s", adr, port)
    sock.close()
